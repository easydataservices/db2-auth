...
BEGIN
  ...
  IF v_auth_name IS NOT NULL THEN
    IF v_last_authenticated_ts IS NULL THEN
      SET v_last_authenticated_ts = p_session_config.change_ts;
    ELSE
      SET v_last_authenticated_ts = MAX(p_session_config.change_ts, v_last_authenticated_ts);
    END IF;
  END IF;
